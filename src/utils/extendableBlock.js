 ...
 * 最新修复：完美的 Blockly 事件兼容、撤销重做支持及深度垃圾回收
 */
...
const defaultPlusSelectImage =
    'data:image/svg+xml;base64,'

const enabledDynamicArgBlocksInfo = {}
const extInfo = {}
const PROXY_MARK = Symbol.for('ExpandableProxyMark')
...
function cleanInputs(runtime, target, blockId, dynamicArgumentIds) {
    ...
    try {
        ...
        Object.keys(block.inputs).forEach(name => {
            if (
                ...
                if (block.inputs[name].shadow) {
                    deleteBlockRecursive(target, block.inputs[name].shadow)
                }
        ...
        console.warn('[cleanInputs] Error:', e)
    }
}
...
function initExpandableBlock(
    ...
    blockDefinition.init = function () {
        ...
        const endTxt = getValue(dynamicArgInfo.endText, 0, '')
        if (endTxt !== '')
    ...
    blockDefinition.attachShadow_ = function (
        ...
        if (argumentType === 'n')
            newBlock.setFieldValue(String(defaultValue), 'NUM')
        else if (argumentType === 's' || argumentType === 'l')
            newBlock.setFieldValue(String(defaultValue), 'TEXT')
    ...
    blockDefinition.removeDynamicArg = function (id) {
        ...
        const name = match[1],
            indexToRem = Number(match[2])
        getParamsGroupindexes.call(this, indexToRem).forEach(it => {
            const curId = `${name}${it}`
...
const patchSymbol = Symbol('ExpandablePatch')
