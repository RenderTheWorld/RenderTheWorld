...
export class JoinEditor extends BaseNodeEditor {
	constructor() {
		...
		const xElement = setInputAestheticsFromType(
			new LabelElement('x | r'),
			'Number'
		).onConnect(update)
		const yElement = setInputAestheticsFromType(
			new LabelElement('y | g'),
			'Number'
		).onConnect(update)
		const zElement = setInputAestheticsFromType(
			new LabelElement('z | b'),
			'Number'
		).onConnect(update)
		const wElement = setInputAestheticsFromType(
