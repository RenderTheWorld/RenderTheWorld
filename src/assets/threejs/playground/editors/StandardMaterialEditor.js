...
export class StandardMaterialEditor extends MaterialEditor {
	constructor() {
		...
		const opacity = setInputAestheticsFromType(
			new LabelElement('opacity'),
			'Number'
		)
		const metalness = setInputAestheticsFromType(
			new LabelElement('metalness'),
			'Number'
		)
		const roughness = setInputAestheticsFromType(
			new LabelElement('roughness'),
			'Number'
		)
		const emissive = setInputAestheticsFromType(
			new LabelElement('emissive'),
			'Color'
		)
		const normal = setInputAestheticsFromType(
			new LabelElement('normal'),
			'Vector3'
		)
		const position = setInputAestheticsFromType(
			new LabelElement('position'),
			'Vector3'
		)
