...
const removeDOMClass = (dom, classList) => {
	if (classList)
		classList.split(' ').forEach(alignClass => dom.classList.remove(alignClass))
}
	...
	connect(element = null) {
		...
		if (element !== null) {
			...
			if (this.disconnectDOM === null) {
				...
				this.disconnectDOM.innerHTML = Element.icons.unlink
					? `<i class='${Element.icons.unlink}'></i>`
					: '✖'
...
class Canvas extends Serializer {
	...
	set zoom(val) {
		...
		val === 1
			? this.dom.classList.remove('zoom')
			: this.dom.classList.add('zoom')
	...
	serialize(data) {
		const nodes = []
		const serializeNodes = this.nodes.sort((a, b) =>
			a.serializePriority > b.serializePriority ? -1 : 1
		)
