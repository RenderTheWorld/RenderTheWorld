...
export const cameraProjectionMatrix = /*@__PURE__*/ Fn(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraProjectionMatrix = cameraProjectionMatrices
			.element(cameraIndex)
 ...
 * @type {UniformNode<mat4>}
 */
export const cameraProjectionMatrixInverse = /*@__PURE__*/ uniform('mat4')
	.label('cameraProjectionMatrixInverse')
	.setGroup(renderGroup)
	.onRenderUpdate(({ camera }) => camera.projectionMatrixInverse)
...
export const cameraViewMatrix = /*@__PURE__*/ Fn(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraViewMatrix = cameraViewMatrices
			.element(cameraIndex)
...
export const cameraWorldMatrix = /*@__PURE__*/ uniform('mat4')
	...
	.setGroup(renderGroup)
	.onRenderUpdate(({ camera }) => camera.matrixWorld)
...
export const cameraNormalMatrix = /*@__PURE__*/ uniform('mat3')
	...
	.setGroup(renderGroup)
	.onRenderUpdate(({ camera }) => camera.normalMatrix)
