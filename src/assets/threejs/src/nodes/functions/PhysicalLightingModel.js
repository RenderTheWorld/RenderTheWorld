...
const getVolumeTransmissionRay = /*@__PURE__*/ Fn(
	([n, v, thickness, ior, modelMatrix]) => {
		// Direction of refracted light.
		const refractionVector = vec3(
			refract(v.negate(), normalize(n), div(1.0, ior))
		)
...
const getTransmissionSample = /*@__PURE__*/ Fn(
	([fragCoord, roughness, ior], { material }) => {
		const vTexture =
...
const volumeAttenuation = /*@__PURE__*/ Fn(
	([transmissionDistance, attenuationColor, attenuationDistance]) => {
		If(attenuationDistance.notEqual(0), () => {
			// Compute light attenuation using Beer's law.
			const attenuationCoefficient = log(attenuationColor)
				.negate()
				.div(attenuationDistance)
		...
		if (dispersion) {
			...
			Loop({ start: 0, end: 3 }, ({ i }) => {
				...
				const ndcPos = projMatrix.mul(
					viewMatrix.mul(vec4(refractedRayExit, 1.0))
				)
			...
			transmittance = diffuseColor.mul(
				volumeAttenuation(
					length(transmissionRay),
					attenuationColor,
					attenuationDistance
...
const IBLSheenBRDF = /*@__PURE__*/ Fn(({ normal, viewDir, roughness }) => {
	...
	const DG = select(
		roughness.lessThan(0.25),
		0.0,
		float(0.1).mul(roughness).sub(0.025)
	).add(a.mul(dotNV).add(b).exp())
...
class PhysicalLightingModel extends LightingModel {
	...
	directRectArea({
		...
		reflectedLight.directSpecular.addAssign(
			lightColor
				.mul(fresnel)
				.mul(LTC_Evaluate({ N, V, P, mInv, p0, p1, p2, p3 }))
		)
	...
	indirectSpecular({ radiance, iblIrradiance, reflectedLight }) {
		...
		if (this.clearcoat === true) {
			const dotNVcc = transformedClearcoatNormalView
