 ...
 */
export const getViewPosition = /*@__PURE__*/ Fn(
	([screenPosition, depth, projectionMatrixInverse], builder) => {
		...
		if (builder.renderer.coordinateSystem === WebGPUCoordinateSystem) {
			...
			clipSpacePosition = vec4(
				vec3(screenPosition.x, screenPosition.y.oneMinus(), depth)
					.mul(2.0)
					.sub(1.0),
				1.0
			)
 ...
 */
export const getScreenPosition = /*@__PURE__*/ Fn(
 ...
 */
export const getNormalFromDepth = /*@__PURE__*/ Fn(
	([uv, depthTexture, projectionMatrixInverse]) => {
		...
		const dpdx = dl
			...
			.select(
				ce.sub(
					getViewPosition(
						uv.sub(vec2(float(1).div(size.x), 0)),
						l1,
				...
				ce
					...
					.add(
						getViewPosition(
							uv.add(vec2(float(1).div(size.x), 0)),
							r1,
							projectionMatrixInverse
						)
					)
		...
		const dpdy = db
			...
			.select(
				ce.sub(
					getViewPosition(
						uv.add(vec2(0, float(1).div(size.y))),
						b1,
						projectionMatrixInverse
					)
				),
				ce
					...
					.add(
						getViewPosition(
							uv.sub(vec2(0, float(1).div(size.y))),
							t1,
							projectionMatrixInverse
						)
					)
