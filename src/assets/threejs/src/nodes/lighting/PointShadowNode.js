...
export const PointShadowFilter = /*@__PURE__*/ Fn(
	({ depthTexture, bd3D, dp, texelSize, shadow }) => {
...
const pointShadowFilter = /*@__PURE__*/ Fn(
	({ filterFn, depthTexture, shadowCoord, shadow }) => {
		...
		If(
			lightToPositionLength
				.sub(cameraFarLocal)
				.lessThanEqual(0.0)
				.and(lightToPositionLength.sub(cameraNearLocal).greaterThanEqual(0.0)),
			() => {
...
class PointShadowNode extends ShadowNode {
	...
	setupShadowFilter(
		builder,
		{ filterFn, shadowTexture, depthTexture, shadowCoord, shadow }
	) {
	...
	renderShadow(frame) {
		...
		for (let vp = 0; vp < viewportCount; vp++) {
			...
			const y =
				_shadowMapSize.y - _viewportSize.y - _viewportSize.y * viewport.y
...
export const pointShadow = (light, shadow) =>
	nodeObject(new PointShadowNode(light, shadow))
