...
export const PCFShadowFilter = /*@__PURE__*/ Fn(
	({ depthTexture, shadowCoord, shadow }) => {
		const depthCompare = (uv, compare) =>
			texture(depthTexture, uv).compare(compare)
	...
	({ depthTexture, shadowCoord }) => {
		...
		If(hardShadow.notEqual(float(1.0)), () => {
			...
			let softnessProbability = variance.div(
				variance.add(distance.mul(distance))
...
const VSMPassVertical = /*@__PURE__*/ Fn(
	({ samples, radius, size, shadowPass }) => {
	...
	({ samples, radius, size, shadowPass }) => {
		...
		Loop(
			...
			({ i }) => {
				...
				const distribution = shadowPass.sample(
...
class ShadowNode extends ShadowBaseNode {
	...
	setupShadowCoord(builder, shadowPosition) {
		...
		if (
			shadow.camera.isOrthographicCamera ||
			...
			const cameraFarLocal = reference('far', 'float', shadow.camera).setGroup(
				renderGroup
			)
	...
	setupShadow(builder) {
		...
		if (shadowMapType === VSMShadowMap) {
			...
			this.vsmShadowMapVertical = builder.createRenderTarget(
				shadow.mapSize.width,
				shadow.mapSize.height,
				{ format: RGFormat, type: HalfFloatType }
			)
			...
			const samples = reference('blurSamples', 'float', shadow).setGroup(
				renderGroup
			)
			const radius = reference('radius', 'float', shadow).setGroup(renderGroup)
		...
		const shadowIntensity = reference('intensity', 'float', shadow).setGroup(
			renderGroup
		)
		const normalBias = reference('normalBias', 'float', shadow).setGroup(
			renderGroup
		)

		const shadowPosition = lightShadowMatrix(light).mul(
			shadowPositionWorld.add(transformedNormalWorld.mul(normalBias))
		)
		...
		const filterFn =
			shadow.filterNode ||
			this.getShadowFilterFn(renderer.shadowMap.type) ||
			null
	...
	updateShadow(frame) {
		...
		renderer.setRenderObjectFunction(
			(object, scene, _camera, geometry, material, group, ...params) => {
				if (
...
export const shadow = (light, shadow) =>
	nodeObject(new ShadowNode(light, shadow))
