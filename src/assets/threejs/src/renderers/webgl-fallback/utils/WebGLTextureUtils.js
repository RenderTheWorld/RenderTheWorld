...
class WebGLTextureUtils {
	...
	setTextureParameters(textureType, texture) {
		...
		const hasMipmaps =
			texture.mipmaps !== undefined && texture.mipmaps.length > 0

		// follow WebGPU backend mapping for texture filtering
		const minFilter =
			texture.minFilter === LinearFilter && hasMipmaps
				? LinearMipmapLinearFilter
				: texture.minFilter
		...
		if (extensions.has('EXT_texture_filter_anisotropic') === true) {
			...
			if (texture.anisotropy > 1) {
				...
				gl.texParameterf(
					...
					Math.min(texture.anisotropy, backend.getMaxAnisotropy())
				)
			}
		}
	}
	...
	updateTexture(texture, options) {
		...
		if (
			...
			textureGPU === undefined /* unsupported texture format */
		)
			return
	...
	async copyTextureToBuffer(texture, x, y, width, height, faceIndex) {
		...
		const target = texture.isCubeTexture
