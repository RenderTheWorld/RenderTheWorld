...
class WebGPUBackend extends Backend {
			...
			for (let i = 0; i < colorAttachments.length; i++) {
				...
				if (renderContext.clearColor) {
					colorAttachment.clearValue =
						i === 0 ? renderContext.clearColorValue : { r: 0, g: 0, b: 0, a: 1 }
	...
	compute(computeGroup, computeNode, bindings, pipeline) {
		...
		if (computeNodeData.dispatchSize === undefined)
			computeNodeData.dispatchSize = { x: 0, y: 1, z: 1 }

		const { dispatchSize } = computeNodeData

		if (computeNode.dispatchCount > maxComputeWorkgroupsPerDimension) {
			dispatchSize.x = Math.min(
				computeNode.dispatchCount,
				maxComputeWorkgroupsPerDimension
			)
			dispatchSize.y = Math.ceil(
				...
				for (let i = 0; i < drawCount; i++) {
					...
					if (hasIndex === true) {
						passEncoderGPU.drawIndexed(
							counts[i],
							count,
							starts[i] / index.array.BYTES_PER_ELEMENT,
							0,
							firstInstance
						)
					} else {
