...
class WGSLNodeBuilder extends NodeBuilder {
	...
	generateTextureDimension(texture, textureProperty, levelSnippet) {
		...
		if (textureData.dimensionsSnippet === undefined)
	...
	isUnfilterable(texture) {
		return (
			...
			(this.isSampleCompare(texture) === false &&
	...
	generateTexture(
		...
		if (texture.isVideoTexture === true) {
			snippet = this._generateVideoSample(
				textureProperty,
				uvSnippet,
				shaderStage
			)
...
`
	...
	enableDirective(name, shaderStage = this.shaderStage) {
		const stage =
			this.directives[shaderStage] || (this.directives[shaderStage] = new Set())
		stage.add(name)
	}
		...
		if (shaderStage === 'vertex' || shaderStage === 'fragment') {
			...
			for (let index = 0; index < varyings.length; index++) {
				...
				if (varying.needsInterpolation) {
					...
					snippets.push(
						`${attributesSnippet} ${varying.name} : ${this.getType(varying.type)}`
					)
				} else if (
					shaderStage === 'vertex' &&
	...
	isCustomStruct(nodeUniform) {
		return (
			nodeUniform.value.isStorageBufferAttribute &&
			nodeUniform.node.structTypeNode !== null
		)
	}
			...
			if (
				...
				if (texture.isCubeTexture === true) {
					...
					const componentPrefix =
						this.getComponentTypeFromTexture(texture).charAt(0)
				...
				uniform.type === 'indirectStorageBuffer'
			) {
				...
				if (this.isCustomStruct(uniform)) {
					bufferSnippets.push(
						`@binding( ${uniformIndexes.binding++} ) @group( ${uniformIndexes.group} ) var<${bufferAccessMode}> ${uniform.name} : ${bufferType};`
					)
				} else {
